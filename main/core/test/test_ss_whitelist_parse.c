...
int main(void) {
  ...
  printf("\n--- Group 2: length rejection ---\n");
  {
    ss_keypath_t kp;
    TEST("len=0 rejected");
    if (ss_keypath_parse(buf_44, 0, &kp)) { FAIL("should reject len 0"); }
    else { PASS(); }
  }
  {
    ss_keypath_t kp;
    TEST("len=19 rejected");
    if (ss_keypath_parse(buf_44, 19, &kp)) { FAIL("should reject len 19"); }
    else { PASS(); }
  }
  {
    ss_keypath_t kp;
    TEST("len=21 rejected");
    if (ss_keypath_parse(buf_44, 21, &kp)) { FAIL("should reject len 21"); }
    else { PASS(); }
  }
  {
    ss_keypath_t kp;
    TEST("len=24 rejected (full keypath with fingerprint)");
    if (ss_keypath_parse(buf_44, 24, &kp)) { FAIL("should reject len 24"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 3: unhardened purpose rejected ---\n");
  {
    ...
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject unhardened purpose"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 4: unhardened coin rejected ---\n");
  {
    ...
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject unhardened coin"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 5: unhardened account rejected ---\n");
  {
    ...
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject unhardened account"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 6: hardened chain rejected ---\n");
  {
    ...
    ss_keypath_t kp;
    TEST("hardened chain");
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject hardened chain"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 7: hardened index rejected ---\n");
  {
    ...
    ss_keypath_t kp;
    TEST("hardened index");
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject hardened index"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 8: unknown purpose rejected ---\n");
  {
    /* purpose 45 hardened: 45 | 0x80000000 = 0x8000002d → LE: 0x2d 0x00 0x00 0x80 */
    unsigned char buf[20];
    memcpy(buf, buf_44, 20);
    buf[0] = 0x2d;  /* purpose 45' */
    ss_keypath_t kp;
    TEST("purpose 45' (unknown)");
    if (ss_keypath_parse(buf, 20, &kp)) { FAIL("should reject purpose 45"); }
    else { PASS(); }
  }
  {
    /* purpose 48 hardened: 48 | 0x80000000 = 0x80000030 → LE: 0x30 0x00 0x00 0x80 */
    unsigned char buf[20];
    memcpy(buf, buf_44, 20);
