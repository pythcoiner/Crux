...
int main(void) {
  ...
  {
    registry_clear();
    bool ok = registry_add_from_string("wpkh_test",
        "wpkh([00000000/84'/0'/0']" XPUB_84 "/<0;1>/*)#d9qwe873",
        STORAGE_FLASH, false);
    ...
    TEST("wpkh: count == 1");
    if (registry_count() == 1) { PASS(); } else { FAIL("wrong count"); }

    const registry_entry_t *e = registry_find_by_id("wpkh_test");
    TEST("wpkh: entry found");
    if (e) { PASS(); } else { FAIL("entry not found"); goto skip_wpkh; }

    TEST("wpkh: my_key_index == 0");
    if (e->my_key_index == 0) { PASS(); } else { FAIL("wrong key index"); }

    TEST("wpkh: num_paths == 2");
    if (e->num_paths == 2) { PASS(); } else { FAIL("wrong num_paths"); }

    TEST("wpkh: origin depth == 3");
    if (e->origin_path_len == 3) { PASS(); } else { FAIL("wrong depth"); }

    TEST("wpkh: origin path[0] == 84'");
    if (e->origin_path[0] == 0x80000054u) { PASS(); } else { FAIL("wrong path[0]"); }

    TEST("wpkh: origin path[1] == 0'");
    if (e->origin_path[1] == 0x80000000u) { PASS(); } else { FAIL("wrong path[1]"); }

    TEST("wpkh: origin path[2] == 0'");
    if (e->origin_path[2] == 0x80000000u) { PASS(); } else { FAIL("wrong path[2]"); }
  ...
  printf("\n--- Group 2: wsh(sortedmulti) ---\n");
  {
    ...
    if (e) { PASS(); } else { FAIL("entry not found"); goto skip_sm; }

    TEST("sortedmulti: my_key_index == 0");
    if (e->my_key_index == 0) { PASS(); } else { FAIL("wrong key index"); }

    TEST("sortedmulti: num_paths == 1");
    if (e->num_paths == 1) { PASS(); } else { FAIL("wrong num_paths"); }

    TEST("sortedmulti: origin depth == 4");
    if (e->origin_path_len == 4) { PASS(); } else { FAIL("wrong depth"); }

    TEST("sortedmulti: origin path[0] == 48'");
    if (e->origin_path[0] == 0x80000030u) { PASS(); } else { FAIL("wrong path[0]"); }
    ...
    if (e->origin_path[3] == 0x80000002u) { PASS(); } else { FAIL("wrong path[3]"); }
  ...
  {
    registry_clear();
    bool ok = registry_add_from_string("tr_test",
        "tr([00000000/86'/0'/0']" XPUB_86 "/<0;1>/*)#wpff3yhl",
        STORAGE_FLASH, false);
    ...
    TEST("tr: entry found");
    if (e) { PASS(); } else { FAIL("entry not found"); goto skip_tr; }
    ...
    if (e->origin_path[0] == 0x80000056u) { PASS(); } else { FAIL("wrong path[0]"); }
  ...
  {
    registry_clear();
    bool ok = registry_add_from_string("ms_test",
    ...
    if (e) { PASS(); } else { FAIL("entry not found"); goto skip_ms; }
  ...
  {
    registry_clear();
    bool ok = registry_add_from_string("no_match",
        "wpkh([11111111/84'/0'/0']" XPUB_84 "/0/*)#cmnyxe9f",
        STORAGE_FLASH, false);

    TEST("no match: returns false");
    if (!ok) { PASS(); } else { FAIL("should have returned false"); }

    TEST("no match: count == 0");
    if (registry_count() == 0) { PASS(); } else { FAIL("count should be 0"); }
  }
