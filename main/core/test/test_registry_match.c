...
static void seed_registry(void) {
  ...
  registry_add_from_string("e0",
      "wpkh([00000000/84'/0'/0']" XPUB_84 "/0/*)",
      STORAGE_FLASH, false);
...
int main(void) {
  ...
  printf("--- Group 1: basic match e[2] ---\n");
  {
    ...
    if (!r) { FAIL("returned NULL"); }
    else if (strncmp(r->id, "e2", 2) != 0) { FAIL("wrong entry"); }
    else if (cursor != 3) { FAIL("cursor not advanced"); }
    else { PASS(); }
  }
  ...
  printf("\n--- Group 2: unknown origin ---\n");
  {
    ...
    static const uint8_t kp[] = {
      0xDE, 0xAD, 0xBE, 0xEF,
      0x63, 0x00, 0x00, 0x80,  /* 99' — not in registry */
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00,
    ...
    if (registry_match_keypath(kp, sizeof kp, &cursor) != NULL)
      { FAIL("should return NULL"); } else { PASS(); }
  }
  ...
  printf("\n--- Group 4: hardened trailing ---\n");
  {
    ...
    static const uint8_t kp_hmp[] = {
      ...
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x80,  /* mp = 0' (hardened!) */
      0x03, 0x00, 0x00, 0x00,
    };
    size_t cursor = 0;
    TEST("hardened mp");
  ...
  printf("\n--- Group 5: mp range ---\n");
  {
    ...
    static const uint8_t kp_mp2[] = {
      ...
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x80,
      0x02, 0x00, 0x00, 0x00,  /* mp=2 — out of range */
      0x03, 0x00, 0x00, 0x00,
    };
    size_t cursor = 0;
    TEST("mp > 1");
  ...
  }
  {
    ...
    static const uint8_t kp_sp_mp1[] = {
      ...
      0x00, 0x00, 0x00, 0x80,
      0x00, 0x00, 0x00, 0x80,
      0x01, 0x00, 0x00, 0x00,  /* mp=1 on single-path descriptor */
      0x03, 0x00, 0x00, 0x00,
    };
    size_t cursor = 0;
  ...
  }
  {
    ...
    registry_entry_t *r = registry_match_keypath(kp_mp_mp1, sizeof kp_mp_mp1, &cursor);
    if (!r) { FAIL("returned NULL"); }
    else if (strncmp(r->id, "e1", 2) != 0) { FAIL("wrong entry"); }
    else { PASS(); }
  }
