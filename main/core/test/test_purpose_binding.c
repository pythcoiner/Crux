...
int main(void) {
  ...
  test_strict("48 + P2WPKH (no whitelist entry) → false", 48, SS_SCRIPT_P2WPKH, false);
  test_strict("99 + P2PKH (unknown purpose)     → false", 99, SS_SCRIPT_P2PKH,  false);
  ...
  test_soft("tr/purpose-84 → WARN",
            "tr([00000000/84'/0'/0']" XPUB_84 ")",
            PSB_WARN);

  /* OK: wpkh outer, purpose 84 (matches convention) */
  test_soft("wpkh/purpose-84 → OK",
            "wpkh([00000000/84'/0'/0']" XPUB_84 ")",
            PSB_OK);

  /* NA: wpkh outer, purpose 99 (not in {44,48,49,84,86}) */
  test_soft("wpkh/purpose-99 → NA",
            "wpkh([00000000/99'/0'/0']" XPUB_84 ")",
            PSB_NA);
