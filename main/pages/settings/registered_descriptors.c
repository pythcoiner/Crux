...
static bool rd_compute_checksum(const char *str, size_t len, char out[9]) {
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    cls = cls * 3 + (int)(pos >> 5);
    if (++clscount == 3) { c = rd_polymod(c, cls); cls = 0; clscount = 0; }
  }
  if (clscount > 0) c = rd_polymod(c, cls);
  for (int i = 0; i < 8; i++) c = rd_polymod(c, 0);
  c ^= 1;
  for (int i = 0; i < 8; i++) out[i] = rd_cksum_charset[(c >> (5 * (7 - i))) & 31];
  out[8] = '\0';
  return true;
}
...
static const char *get_type_badge(const struct wally_descriptor *desc) {
  if (!desc) return "?";
  char *canon = NULL;
  if (wally_descriptor_canonicalize(desc, WALLY_MS_CANONICAL_NO_CHECKSUM, &canon) != WALLY_OK)
    return "?";
  const char *badge;
  if      (strncmp(canon, "sh(wpkh(", 8) == 0) badge = "sh-wpkh";
  else if (strncmp(canon, "wpkh(",    5) == 0) badge = "wpkh";
  else if (strncmp(canon, "wsh(",     4) == 0) badge = "wsh-multi";
  else if (strncmp(canon, "tr(",      3) == 0) badge = "tr";
  else if (strncmp(canon, "pkh(",     4) == 0) badge = "pkh";
  else                                          badge = "custom";
...
static bool entry_to_string(const registry_entry_t *entry, char **output) {
  if (!entry || !entry->desc || !output) return false;
  char *canon = NULL;
  ...
  if (!result) { wally_free_string(canon); return false; }
...
static void view_descriptor_cb(void) {
  ...
  if (!entry) return;
  char *desc_str = NULL;
  if (!entry_to_string(entry, &desc_str)) return;
...
static void remove_confirmed_cb(bool confirmed, void *user_data) {
  (void)user_data;
  if (!confirmed || pending_remove_index < 0) return;
...
static void rd_back_cb(void) {
  if (return_callback) return_callback();
}

static void build_rd_menu(void) {
  if (rd_menu) { ui_menu_destroy(rd_menu); rd_menu = NULL; }
...
void registered_descriptors_page_create(lv_obj_t *parent, void (*return_cb)(void)) {
  if (!parent) return;
...
void registered_descriptors_page_show(void) {
  if (rd_screen) lv_obj_clear_flag(rd_screen, LV_OBJ_FLAG_HIDDEN);
  if (rd_menu)   ui_menu_show(rd_menu);
}

void registered_descriptors_page_hide(void) {
  if (rd_screen) lv_obj_add_flag(rd_screen, LV_OBJ_FLAG_HIDDEN);
  if (rd_menu)   ui_menu_hide(rd_menu);
}

void registered_descriptors_page_destroy(void) {
  if (rd_menu)   { ui_menu_destroy(rd_menu); rd_menu = NULL; }
  if (rd_screen) { lv_obj_del(rd_screen); rd_screen = NULL; }
  pending_remove_index = -1;
