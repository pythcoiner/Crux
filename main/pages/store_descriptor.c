...
static void do_save_encrypted(void) {
  ...
  if (ret == ESP_OK) {
    ...
    if (!registry_add_from_string(saved_id, descriptor_text, target_location, false)) {
      ESP_LOGW(TAG, "registry_add_from_string failed for '%s'; will retry at boot", saved_id);
    }
    const char *loc_name =
...
static void do_save_plaintext(const char *id) {
  ...
  if (ret == ESP_OK) {
    registry_remove(id);
    if (!registry_add_from_string(id, descriptor_text, target_location, false)) {
      ESP_LOGW(TAG, "registry_add_from_string failed for '%s'; will retry at boot", id);
    }
    const char *loc_name =
        (target_location == STORAGE_FLASH) ? "flash" : "SD card";
    char msg[64];
